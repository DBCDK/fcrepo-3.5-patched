...
public class DefaultDOManagerTest
{
    ...
    @Mock
    private FieldSearchResult mockFieldSearchResult;
...
//            .thenReturn(mockExistsStmt);
    ...
    public void testGetIngestWriterSucceeds()
        ...
        conditions.add(new Condition("pid", Operator.EQUALS, DUMMY_PID));

        when(mockFieldSearchResult.getCompleteListSize()).thenReturn(0L).thenReturn(1L);
        when(mockFieldSearch.findObjects(
                any(String[].class), any(Integer.TYPE), any(FieldSearchQuery.class))).thenReturn(mockFieldSearchResult, mockFieldSearchResult);
    ...
    public void testGetIngestWriterThrowsIfPidAlreadyRegistered() throws Exception
    {
        ...
        when(mockFieldSearchResult.getCompleteListSize()).thenReturn(1L);
        when(mockFieldSearch.findObjects(
                any(String[].class), any(Integer.TYPE), any(FieldSearchQuery.class))).thenReturn(mockFieldSearchResult);
    ...
    public void testGetIngestWriterThrowsIfObjectAlreadyExists() throws Exception
    {
        ...
        when(mockFieldSearchResult.getCompleteListSize()).thenReturn(0L);
        when(mockFieldSearch.findObjects(
                any(String[].class), any(Integer.TYPE), any(FieldSearchQuery.class))).thenReturn(mockFieldSearchResult);
    ...
    public void testMultithreadedThreadSwitchesBetweenCheckAndRegisterObject() throws Throwable {
        ...
        when(mockFieldSearchResult.getCompleteListSize()).thenAnswer(new Answer<Integer>(){
            @Override
        ...
        when(mockFieldSearch.findObjects(any(String[].class), any(Integer.TYPE), any(FieldSearchQuery.class)))
                .thenReturn(mockFieldSearchResult, mockFieldSearchResult);
        ...
        }).when(mockFieldSearch).update(any(DOReader.class));
    ...
    class ThreadSwitchRunnable implements Runnable {
    	...
    	public void run() {
          ...
          try {
              ...
              LOGGER.info( "{} - Getting ingest writer", Thread.currentThread().getName() );
              DOWriter ingestWriter = manager.getIngestWriter(Server.USE_DEFINITIVE_STORE, mockContext, in, FORMAT, ENCODING, DUMMY_PID);
              LOGGER.info( "{} - Comitting", Thread.currentThread().getName() );
              ingestWriter.commit( "" );
              LOGGER.info( "{} - Release ingest writer", Thread.currentThread().getName() );
