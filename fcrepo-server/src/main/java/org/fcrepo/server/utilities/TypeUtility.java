...
public abstract class TypeUtility {
    ...
    private static org.fcrepo.server.types.gen.FieldSearchResult.ResultList convertSearchObjectFieldsListToGenObjectFieldsArray(List<org.fcrepo.server.search.ObjectFields> sfList) {
        ...
        for (int i = 0; i < sfList.size(); i++) {
            ...
            if (!sf.relObjs().isEmpty()) {
                gf.getRelObj().addAll(toStringList(sf.relObjs()));
            }
            if (!sf.relPredObjs().isEmpty()) {
                gf.getRelPredObj().addAll(toStringList(sf.relPredObjs()));
            }
            if (!sf.relSysPredObjs().isEmpty()) {
                gf.getRelSysPredObj().addAll(toStringList(sf.relSysPredObjs()));
            }
            // Dublin core fields
            if (sf.titles().size() != 0) {
